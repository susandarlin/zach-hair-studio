...
- **Global scope**: `C:/repos/vct/zach-hair-studio/.claude/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `C:/repos/vct/zach-hair-studio/.claude/notes/`)
...
2. Glob `C:/repos/vct/zach-hair-studio/.claude/notes/*.md` (if directory exists) — global notes
...
Global (C:/repos/vct/zach-hair-studio/.claude/notes/):
...
2. **No `.planning/`**: Falls back to global `C:/repos/vct/zach-hair-studio/.claude/notes/` — works in any directory
